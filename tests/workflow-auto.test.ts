...
describe("main loop auto workflow guardrail", () => {
  ...
  it("auto-handles self-contained task templates", () => {
    const selection = selectWorkflowForTask("找几个不同角色脑暴一个产品方向");
    expect(shouldAutoHandleWorkflow("找几个不同角色脑暴一个产品方向", selection)).toBe(true);
  });

  it("does not intercept built-in research/code/kb tasks in the main loop", () => {
    expect(shouldAutoHandleWorkflow("调研一下热门 agent workflow", selectWorkflowForTask("调研一下热门 agent workflow"))).toBe(false);
    expect(shouldAutoHandleWorkflow("修复这个 bug 并运行测试", selectWorkflowForTask("修复这个 bug 并运行测试"))).toBe(false);
    expect(shouldAutoHandleWorkflow("把客服问答整理成知识库", selectWorkflowForTask("把客服问答整理成知识库"))).toBe(false);
  });
